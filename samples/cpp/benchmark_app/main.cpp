...
*/
        ...
        while ((niter != 0LL && iteration < niter) ||
            ...
            //}
            for (auto& output : compiledModel.outputs()) {
                //std::cout << "Output: " << output.get_any_name() << std::endl;
                auto outputTensor = inferRequest->get_tensor(output.get_any_name());
                test += ((float*)outputTensor.data())[0] + 0.001f;
                ov::Tensor kk(outputTensor.get_element_type(), outputTensor.get_shape());
                std::memcpy((char*)kk.data(), (char*)outputTensor.data(), outputTensor.get_byte_size());
                lastOutput = kk;
            }
            ...
            if (!inferenceOnly) {
                ...
                for (auto& item : inputs) {
                    ...
                    ov::Tensor clonedData(data.get_element_type(), data.get_shape());
                    std::memcpy((char*)clonedData.data(), (char*)data.data(), data.get_byte_size());
                    inferRequest->set_tensor(inputName, clonedData);
                    //std::cout << "set_tensor " << inputName << "\n";
                }
/*
                if (useGpuMem) {
                    auto outputTensors =
                    ...
                    for (auto& output : compiledModel.outputs()) {
                        inferRequest->set_tensor(output.get_any_name(), outputTensors[output.get_any_name()]);
                        std::cout << "set_tensor gpu_mem " << output.get_any_name() << "\n";
                    }
                }
*/
            }
            ...
            if (FLAGS_api == "sync") {
                ...
                // well, but as it uses just error codes it has no details like â€˜what()â€™
        ...
        std::cout << "Test: " << test << " " << ((float*)(lastOutput.data()))[0] << std::endl;
